...
class PointCloudManger(object):
    ...
    def get_next_pcd(self) -> None:
        ...
        if self.pcds_left():
            ...
            logging.warn("No point clouds left!")
    ...
    def save_labels_into_file(self, bboxes: List[BBox]) -> None:
        if self.pcds:
            ...
            logging.warn("No point clouds to save labels for!")
    ...
    def load_pointcloud(self, path_to_pointcloud: str) -> PointCloud:
        ...
        start_section(f"Loading {ntpath.basename(path_to_pointcloud)}")
    ...
    def rotate_pointcloud(
        ...
        if abs(pcd_mins[2]) > pcd_maxs[2]:
            logging.warn("Point cloud is upside down, rotating ...")
